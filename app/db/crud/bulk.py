...
async def _resolve_target_user_ids(db: AsyncSession, bulk_model: BulkGroup) -> set[int]:
    ...
    if bulk_model.hase_group_ids:
        result = await db.execute(select(User.id).where(User.groups.any(Group.id.in_(bulk_model.hase_group_ids))))
        user_ids.update({row[0] for row in result.all()})

    if not bulk_model.users and not bulk_model.admins and not bulk_model.hase_group_ids:
