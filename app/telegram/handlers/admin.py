...
def inbound_command(call: types.CallbackQuery):
    bot.edit_message_text(
        ...
        call.message.message_id,
        parse_mode="markdown",
        reply_markup=BotKeyboard.inbounds_menu(call.data, core.config._inbounds_by_tag),
    )
...
<b>Add template <u>Data limit</u> and <u>Time</u> to user or Reset to <u>Template default</u></b>⁉️""",
            ...
            db_user = crud.update_user(db, db_user, modify)
            core.operations.add_user(db_user)
...
def select_protocols(call: types.CallbackQuery):
    ...
    if protocol in protocols:
        ...
        protocols.update({protocol: [inbound["tag"] for inbound in core.config.inbounds_by_protocol[protocol]]})
    mem_store.set(f"{call.message.chat.id}:protocols", protocols)
...
def confirm_user_command(call: types.CallbackQuery):
    ...
    if data == "delete":
        ...
        with GetDB() as db:
            ...
            core.operations.remove_user(db_user)
...
<b>By :</b> <a href="tg://user?id={chat_id}">{full_name}</a>"""
        ...
        config = core.config.include_db_users()
        core.core.restart(config)
        for node_id, node in list(core.nodes.items()):
            if node.connected:
                core.operations.restart_node(node_id, config)
        bot.edit_message_text(
        ...
        with GetDB() as db:
            ...
            for protocol in core.config.inbounds_by_protocol:
...
"""
        ...
        with GetDB() as db:
            ...
            if user.status == UserStatus.active:
                core.operations.update_user(db_user)
...
<b>New Proxies :</b> <code>{", ".join(list(user.inbounds.values())[0])}</code>{tag}"""
        ...
        for i in range(number):
            ...
            for proxy_type in new_user.proxy_settings:
                if not core.config.inbounds_by_protocol.get(proxy_type):
            ...
            with open(file_name, "w") as f:
                ...
                for user in depleted_users:
                    try:
                        ...
                        core.operations.remove_user(user)
                        deleted += 1
                        f.write(
                            f"{user.username}\
...
<b>By :</b> <a href="tg://user?id={chat_id}">{full_name}</a>"""
        ...
        with GetDB() as db:
            ...
            for user in users:
                ...
                protocol = core.config._inbounds_by_tag[inbound]["protocol"]
                ...
                if (data == "inbound_remove" and inbound in inbound_tags) or (
                    ...
                    try:
                        ...
                        if user.status == UserStatus.active:
                            core.operations.update_user(user)
                    except Exception:
                        db.rollback()
                        unsuccessful += 1
