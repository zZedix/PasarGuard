...
def data_usage_percent_reached(
    ...
    if NOTIFY_IF_DATA_USAGE_PERCENT_REACHED:
        ...
        create_notification_reminder(
            db, ReminderType.data_usage, expires_at=expire if expire else None, user_id=user_id, threshold=threshold
        )
...
def expire_days_reached(db: AsyncSession, days: int, user: UserResponse, user_id: int, expire: dt, threshold=None) -> None:
    ...
    if NOTIFY_IF_DAYS_LEFT_REACHED:
        create_notification_reminder(
            db, ReminderType.expiration_date, expires_at=expire, user_id=user_id, threshold=threshold
        )
