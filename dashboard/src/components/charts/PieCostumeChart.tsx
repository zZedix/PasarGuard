    ...
    <Card className="w-full sm:max-w-md">
      ...
      <CardContent className="flex-1 pb-0">
        {isLoading ? (
          ...
          <div className="aspect-square max-h-[250px] flex items-center justify-center text-destructive">{t('errors.failedToLoad')}</div>
        ) : nodesData.length === 0 ? (
          <div className="aspect-square max-h-[250px] flex items-center justify-center">{t('statistics.noNodesAvailable')}</div>
        ) : (
          <ChartContainer config={chartConfig} className="mx-auto aspect-square max-h-[250px]">
            <PieChart>
              <ChartTooltip cursor={false} content={<ChartTooltipContent formatter={value => formatBytes(Number(value), 1)} />} />
              <Pie data={nodesData} dataKey="usage" nameKey="node" outerRadius={80} label={({ node, value }) => `${node}: ${formatBytes(value, 1)}`} />
            </PieChart>
          </ChartContainer>
        )}
        <div className="my-4 flex flex-wrap justify-center gap-4">
          {nodesData.map(entry => (
            <div key={entry.nodeId} className="flex items-center gap-2">
              <div className="h-3 w-3 border rounded-sm" style={{ backgroundColor: entry.fill }} />
