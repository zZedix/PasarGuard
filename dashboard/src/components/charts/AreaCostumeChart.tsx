    ...
    <Card className={'flex flex-1 flex-col'}>
      <CardHeader className="flex flex-col items-stretch space-y-0 border-b p-0 sm:flex-row">
        <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-5 sm:py-6 border-b">
          ...
          <CardDescription>{t('statistics.serverPerformance')}</CardDescription>
        </div>
        <div className=" m-0 p-6 flex flex-col justify-center px-4 sm:border-b-0 sm:border-l border-b">
          ...
          <span className="text-foreground text-lg flex justify-center">{displayCpuUsage}</span>
        </div>
      ...
      <CardContent className={'pt-8'}>
        {isLoading ? (
          ...
          <div className="max-h-[360px] min-h-[200px] w-full flex items-center justify-center text-destructive">{t('errors.failedToLoad')}</div>
        ) : (
          <ChartContainer dir={'ltr'} config={chartConfig} className={'max-h-[360px] min-h-[200px] w-full'}>
            <AreaChart accessibilityLayer data={statsHistory}>
              <defs>
